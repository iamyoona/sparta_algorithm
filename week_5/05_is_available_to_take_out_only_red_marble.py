...
# 모든 경우를 탐색 가능한 BFS로 탐색
# 방문 여부를 저장하기 위한 visited 있어야되는데, 공이 두개??
# 4차원 배열 사용하면 됨
# visited[red_marble_row][red_marble_col][blue_marble_row][blue_marble_col]
# 3 <= x <= 10 (보드의 크기는 상수로 봐도 무방할정도의 크기로 제한돼있음)

dr = [-1, 0, 1, 0]
dc = [0, 1, 0, -1]


def move_until_wall_or_home(r, c, diff_r, diff_c, game_map):
    move_count = 0
...
def is_available_to_take_out_only_red_marble(game_map):
    ...
    while queue:
        ...
        for i in range(4):
            next_red_row, next_red_col, r_count = move_until_wall_or_home(red_row, red_col, dr[i], dc[i], game_map)
            next_blue_row, next_blue_col, b_count = move_until_wall_or_home(blue_row, blue_col, dr[i], dc[i], game_map)
